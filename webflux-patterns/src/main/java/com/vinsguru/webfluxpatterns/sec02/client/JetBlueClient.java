...
public class JetBlueClient {
  ...
  public JetBlueClient(@Value("${sec02.jetblue.service}") final String baseUrl) {
    this.client = WebClient.builder().baseUrl(baseUrl).build();
  }

  public Flux<FlightResult> getFlights(final String from, final String to) {
    return this.client
        .get()
        .uri("{from}/{to}", from, to)
        .retrieve()
  ...
  private void normalizeResponse(final FlightResult result, final String from, final String to) {
    result.setFrom(from);
    result.setTo(to);
